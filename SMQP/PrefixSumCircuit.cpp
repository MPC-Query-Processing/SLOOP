...
BetaCircuit OR() { // ret out <- a or b
    BetaCircuit r;
    BetaBundle a(64), b(64), axorb(64), aandb(64), out(64);
    r.addInputBundle(a);
    r.addInputBundle(b);
    r.addTempWireBundle(axorb);
    r.addTempWireBundle(aandb);
    r.addOutputBundle(out);

    // a or b = (a xor b) xor (a and b)
    for (auto i=0; i<64; ++i) {
        r.addGate(a[i], b[i], GateType::Xor, axorb[i]);
        r.addGate(a[i], b[i], GateType::And, aandb[i]);
    }

    for (auto i=0; i<64; ++i) {
        r.addGate(axorb[i], aandb[i], GateType::Xor, out[i]);
    }
    return r;
}

void SegmentedPrefixOr(u64 partyIdx, DBServer& srvs, const sbMatrix key, sbMatrix& val) {
    u64 nrows = key.rows();
    BetaLibrary lib;
    auto circ_eqc_key = EqualityCheckAndMuxDepthOptimized();
    auto circ_add_val = OR();
    // using "aby3::CircuitLibrary::Optimized::Size" (default setting) has smaller size but larger depth.
...
BetaCircuit SingleEqualityCheckAndMux() { // ret c <- (k0 == k1) ? p0 : p1
    BetaCircuit r;
    BetaBundle a0(64), a1(64), k0(64), k1(64);
    BetaBundle t(64), ta0(64), nta1(64), a(64);
    r.addInputBundle(a0);
    r.addInputBundle(a1);
    r.addInputBundle(k0);
    r.addInputBundle(k1);
...
void SegmentedCopy(u64 partyIdx, DBServer& srvs, const sbMatrix key, sbMatrix& value1) {
    u64 nrows = key.rows();
    BetaLibrary lib;
    auto circ = SingleEqualityCheckAndMux();
    Sh3BinaryEvaluator eval;
    
    u64 jump = 1;
    for (; jump < nrows; jump *= 2) {
        u64 size = nrows / (jump * 2);
        sbMatrix a0(size, 64), a1(size, 64), k0(size, 64), k1(size, 64), a(size, 64);
        u64 counter = 0;
        for (auto idx = jump - 1; idx + jump < nrows; idx += jump * 2) {
